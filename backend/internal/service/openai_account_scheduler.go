...
func (s *defaultOpenAIAccountScheduler) tryPreferredProbe(
	...
	for _, preferred := range preferredList {
		fresh := s.service.resolveFreshSchedulableOpenAIAccount(ctx, preferred, req.Platform, req.RequestedModel, req.RequireCompact, req.RequiredCapability)
		if fresh == nil {
			continue
		}
		fresh = s.service.recheckSelectedOpenAIAccountFromDB(ctx, fresh, req.Platform, req.RequestedModel, req.RequireCompact, req.RequiredCapability)
