...
func (r *channelRepository) ListModelPricing(ctx context.Context, channelID int64) ([]service.ChannelModelPricing, error) {
	rows, err := r.db.QueryContext(ctx,
		`SELECT id, channel_id, platform, models, billing_mode, input_price, output_price, cache_write_price, cache_read_price, image_input_price, image_output_price, per_request_price, service_tier_multipliers, created_at, updated_at
...
func (r *channelRepository) UpdateModelPricing(ctx context.Context, pricing *service.ChannelModelPricing) error {
	...
	result, err := r.db.ExecContext(ctx,
		`UPDATE channel_model_pricing
		 SET models = $1, billing_mode = $2, input_price = $3, output_price = $4, cache_write_price = $5, cache_read_price = $6, image_input_price = $7, image_output_price = $8, per_request_price = $9, platform = $10, service_tier_multipliers = $11, updated_at = NOW()
		 WHERE id = $12`,
		modelsJSON, billingMode, pricing.InputPrice, pricing.OutputPrice, pricing.CacheWritePrice, pricing.CacheReadPrice,
		pricing.ImageInputPrice, pricing.ImageOutputPrice, pricing.PerRequestPrice, pricing.Platform, serviceTierMultipliersJSON(pricing.ServiceTierMultipliers), pricing.ID,
	)
	if err != nil {
...
func scanModelPricingRows(rows *sql.Rows) ([]service.ChannelModelPricing, []int64, error) {
	...
	for rows.Next() {
		...
		var modelsJSON []byte
		var serviceTierMultipliersJSON []byte
		if err := rows.Scan(
			&p.ID, &p.ChannelID, &p.Platform, &modelsJSON, &p.BillingMode,
			&p.InputPrice, &p.OutputPrice, &p.CacheWritePrice, &p.CacheReadPrice,
			&p.ImageInputPrice, &p.ImageOutputPrice, &p.PerRequestPrice, &serviceTierMultipliersJSON, &p.CreatedAt, &p.UpdatedAt,
		); err != nil {
		...
		p.ServiceTierMultipliers = parseServiceTierMultipliersJSON(serviceTierMultipliersJSON)
...
func serviceTierMultipliersJSON(multipliers map[string]float64) []byte {
	normalized := service.NormalizeServiceTierMultipliers(multipliers)
	if normalized == nil {
		return []byte(`{}`)
	}
...
func parseServiceTierMultipliersJSON(data []byte) map[string]float64 {
	if len(data) == 0 {
		return nil
	}
	...
	return service.NormalizeServiceTierMultipliers(multipliers)
}
...
func createModelPricingExec(ctx context.Context, exec dbExec, pricing *service.ChannelModelPricing) error {
	...
	err = exec.QueryRowContext(ctx,
		`INSERT INTO channel_model_pricing (channel_id, platform, models, billing_mode, input_price, output_price, cache_write_price, cache_read_price, image_input_price, image_output_price, per_request_price, service_tier_multipliers)
		...
		pricing.ImageInputPrice, pricing.ImageOutputPrice, pricing.PerRequestPrice, serviceTierMultipliersJSON(pricing.ServiceTierMultipliers),
